...
from ananas.projects.models import Server, Application
...
import feedparser
import couchdb
from ananas.projects import tester
...
def test(request):
    ...
    if not request.user.is_authenticated():
        ...
        for i in cord:
            dat.append([i[0], i[1]])
        print dat
        return render_to_response('projects/test_data.html',{'datarec':dat} )
...
def dual(request):
    ...
    if not request.user.is_authenticated():
        ...
        return render_to_response('projects/examples/dual-axis.html',{} )
